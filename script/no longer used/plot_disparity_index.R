...
###########################################
shape <- readOGR(dsn = "./data/summarization/TEOW", layer = "wwf_terr_ecos")  # read the shapefile in by name not the lack of .shp extension
...
shape_MEOW <- readOGR(dsn = "./data/summarization/MEOW_2", layer = "WCMC-036-MEOW-PPOW-2007-2012-NoCoast")  # read the shapefile in by name not the lack of .shp extension
...
##### COUNTRY 
orders_from_joyplot <- fct_reorder(intersection$geounit,intersection$disparity, .fun = mean) %>% levels()
colors_for_joy <- colorRampPalette(brewer.pal(8,"Spectral"))(24) %>% sample()

# box plot
MEOW_joy <- intersection %>%
  dplyr::select(disparity, geounit) %>% 
  mutate(geounit = fct_relevel(geounit,orders_from_joyplot %>% rev())) %>% 
  ggplot()+
  #annotate("rect", xmin = -15, xmax = 0, ymin = -50, ymax = 1500, fill = "red4", alpha = .5)+
  geom_histogram(aes(x = disparity, fill = subregion), show.legend = F, bins = 70) +
  geom_vline(xintercept = 0, lty = "dashed", size = .5)+
  #scale_fill_gradient2(low = "Red2", high = "Black")+
  scale_fill_manual(values = colors_for_joy)+
  facet_wrap(~geounit,ncol = 4, scales = "free_y")+
  theme_minimal()+
  theme(axis.title.y = element_blank()
  )
...
marine_realm <- shape_WORLD %>% 
  ...
  mutate(geounit = fct_relevel(geounit,orders_from_joyplot %>% rev())) %>% 
  ggplot() +
  geom_sf( aes(group = geounit, fill =  geounit)
           ,color = "NA", show.legend = F)+
  scale_fill_manual(values = colors_for_joy )+
  theme_void()+
  theme(panel.grid.major = element_line(colour = "white"))
